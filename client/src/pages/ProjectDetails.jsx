...
const ProjectDetails = () => {
    ...
    const filterTask = (input) => {
        if(input === "ALL") {
            setFilterTasks(null)
        } else {
            const data = subtasks.filter(subtask => subtask.status === input)
            setFilterTasks(data)
        }
    }

    const editSingleSubTask = (subtask) => {
        ...
        <div className='project__details-container'>
            <div className="project__info">
                <div className="project__info-text">
                    {((subtasks.filter((subtask) => subtask.status === "DONE")).length / subtasks.length) !== 1 ? (
                        <h3 className="incomplete">IN PROGRESS {(subtasks.filter((subtask) => subtask.status === "DONE")).length} / {subtasks.length}</h3>
                    ) : (
            ...
            <div className="subtasks__container">
                ...
                <div className="filter__container">
                    <button className='btn-alert' onClick={() => filterTask("IN PROGRESS")}>InProgress - {(subtasks.filter((subtask) => subtask.status === "IN PROGRESS")).length}</button>
                    <button className='btn-alert' onClick={() => filterTask("DONE")}>Done - {(subtasks.filter((subtask) => subtask.status === "DONE")).length}</button>
